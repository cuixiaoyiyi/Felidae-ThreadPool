...
//			return;
	...
	public synchronized static void recordIL(KeyPoint newPoint) {
	//	if (!needRecord(ILSet, newPoint, null)) {
	//		return;
	//	}
	
		String filePath = ERROR_FOLDER + "IL-sum.txt";
		recordSum(newPoint, null, filePath);
	}
	
	public static void recordCallerRunsChecker(KeyPoint point) {
		String filePath = ERROR_FOLDER + "CallerRuns-sum.txt";
		recordSum(point, null, filePath);
	}
	
	public static void recordNonExceptionHandlerChecker(InitPoint point) {
		String filePath = ERROR_FOLDER + "NonExceptionHandler-sum.txt";
		recordSum(point, null, filePath);
	}


	public static String getObjectKey(KeyPoint newPoint, SootClass sootClass) {
		return newPoint.getMethod() + ";" + newPoint.getStmt() + ";" + sootClass;
	}
